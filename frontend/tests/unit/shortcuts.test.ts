...
import * as shortcuts from "../../src/features/shortcuts/shortcuts.ts";
import type { GuideSection } from "../../src/features/shortcuts/shortcuts.ts";

// legacy tests used string keys ("Ctrl O"); bindingGroups still accepts strings
const ITEMS = [
  {
    cat: "General",
    items: [
...
test("production command metadata and derived guide sections are immutable", () => {
  ...
  assert.throws(
    () => (shortcuts.commands[0]!.bindings as string[]).push("Alt+O"),
    TypeError,
  );
});
...
test("binding matching requires exact modifiers", () => {
  assert.equal(
    shortcuts.matchesBinding(keyEvent("o", { ctrlKey: true }), "Ctrl+O"),
    true,
  );
  assert.equal(
  ...
  assert.equal(
    shortcuts.matchesBinding(keyEvent("S", { altKey: true }), "S"),
    false,
  );
  assert.equal(
    shortcuts.matchesBinding(keyEvent("?", { shiftKey: true }), "?"),
    true,
  );
});
...
test("a registry validates action registration", () => {
  ...
  assert.throws(
    () => registry.registerAction("missing", () => {}),
    /Unknown command/,
  );
...
test("shortcut search includes structured alternatives and usage notes", () => {
  const structuredItems: GuideSection[] = [
    {
      cat: "Edit",
      items: [
        {
...
test("shortcut sections stack independently within their assigned columns", () => {
  const sections: GuideSection[] = [
    { cat: "General", column: 0, items: [] },
    { cat: "Edit", column: 1, items: [] },
    { cat: "Playback & navigation", column: 0, items: [] },
    { cat: "Timeline & segments", column: 1, items: [] },
  ];
...
class ButtonShell {
  attributes = new Set<string>();
  classes = new Set(["kbs-clear", "is-hidden"]);
  classList = {
...
test("replacing the shortcuts dialog clears its state without restoring stale focus", () => {
  ...
  const nextReturnFocus = shortcuts.cleanupDialog(
    box as unknown as Parameters<typeof shortcuts.cleanupDialog>[0],
    trap as unknown as (event: KeyboardEvent) => void,
    returnFocus,
    false,
  );
...
test("editor shortcuts are disabled while a modal or settings page is open", () => {
  function documentWithState(modalOpen: boolean, settingsOpen: boolean) {
    return {
      body: {
        classList: {
          contains: (name: string) => name === "settings-open" && settingsOpen,
        },
      },
      getElementById: (id: string) =>
        id === "mod-overlay"
          ? {
              classList: {
                contains: (name: string) => name === "open" && modalOpen,
              },
            }
          : null,
    };
  ...
  assert.equal(
    shortcuts.editorCommandsEnabled(documentWithState(false, false)),
    true,
  );
  assert.equal(
    shortcuts.editorCommandsEnabled(documentWithState(true, false)),
    false,
  );
  assert.equal(
    shortcuts.editorCommandsEnabled(documentWithState(false, true)),
    false,
  );
});
