...
import * as audio from "../../src/features/audio/audio-core.ts";
import type { AudioClip } from "../../src/features/audio/audio-core.ts";
...
test("setting an audio fragment edge to the playhead preserves source alignment", () => {
  ...
  const start = audio.trimClipToTimelinePoint(clip, "start", 5, 0, 12, 20, 0.05);
  const end = audio.trimClipToTimelinePoint(clip, "end", 7, 0, 12, 20, 0.05);
...
test("audio playhead trims respect neighbors, source bounds, and minimum duration", () => {
  ...
  const extendedEnd = audio.trimClipToTimelinePoint(clip, "end", 15, 0, 12, 10, 0.05);
  const minimumEnd = audio.trimClipToTimelinePoint(clip, "end", 2, 0, 12, 10, 0.05);
...
test("dragging the detail waveform slips audio beneath the fixed frame", () => {
  assert.ok(
    Math.abs(
      audio.sourceRangeDetailDragValue(51.571, -30, 21.9825, 270, 248.461) - 54.0135,
    ) < 1e-9,
  );
  assert.ok(
    Math.abs(
      audio.sourceRangeDetailDragValue(51.571, 30, 21.9825, 270, 248.461) - 49.1285,
    ) < 1e-9,
  );
